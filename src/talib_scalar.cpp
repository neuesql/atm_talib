...
// Signature: TA_FUNC(startIdx, endIdx, inReal[], optInTimePeriod, &outBegIdx,
...
template <TA_RetCode (*TA_FUNC)(int, int, const double[], int, int *, int *,
                                double[])>
static void TalibScalarP1Double(DataChunk &args, ExpressionState &state,
                                Vector &result) {
  ...
  for (idx_t i = 0; i < count; i++) {
    ...
    TA_RetCode rc = TA_FUNC(0, size - 1, input.data(), period, &outBeg, &outNb,
                            outReal.data());
    if (rc != TA_SUCCESS) {
      outNb = 0;
      outBeg = 0;
...
template <TA_RetCode (*TA_FUNC)(int, int, const double[], int, int *, int *,
                                int[])>
static void TalibScalarP1Int(DataChunk &args, ExpressionState &state,
                             Vector &result) {
  auto count = args.size();
  for (idx_t i = 0; i < count; i++) {
    auto list = GetListEntry(args, 0, i);
...
// Signature: TA_FUNC(startIdx, endIdx, inReal[], &outBegIdx, &outNBElement,
// outReal[])
// ============================================================

// P2 returning DOUBLE
template <TA_RetCode (*TA_FUNC)(int, int, const double[], int *, int *,
                                double[])>
static void TalibScalarP2Double(DataChunk &args, ExpressionState &state,
                                Vector &result) {
  auto count = args.size();
  for (idx_t i = 0; i < count; i++) {
    auto list = GetListEntry(args, 0, i);
    ...
    std::vector<double> outReal(size);
    TA_RetCode rc =
        TA_FUNC(0, size - 1, input.data(), &outBeg, &outNb, outReal.data());
    if (rc != TA_SUCCESS) {
      outNb = 0;
      outBeg = 0;
    }
...
// P2 returning INT
template <TA_RetCode (*TA_FUNC)(int, int, const double[], int *, int *, int[])>
static void TalibScalarP2Int(DataChunk &args, ExpressionState &state,
                             Vector &result) {
  auto count = args.size();
  for (idx_t i = 0; i < count; i++) {
    auto list = GetListEntry(args, 0, i);
    ...
    std::vector<int> outInt(size);
    TA_RetCode rc =
        TA_FUNC(0, size - 1, input.data(), &outBeg, &outNb, outInt.data());
    if (rc != TA_SUCCESS) {
      outNb = 0;
      outBeg = 0;
    }
...
// Signature: TA_FUNC(startIdx, endIdx, inHigh[], inLow[], inClose[],
// optInTimePeriod, &outBeg, &outNb, outReal[])
// ============================================================
template <TA_RetCode (*TA_FUNC)(int, int, const double[], const double[],
                                const double[], int, int *, int *, double[])>
static void TalibScalarP3(DataChunk &args, ExpressionState &state,
                          Vector &result) {
  auto count = args.size();
  for (idx_t i = 0; i < count; i++) {
    ...
    int outBeg = 0, outNb = 0;
    std::vector<double> outReal(size);
    TA_RetCode rc = TA_FUNC(0, size - 1, high.data(), low.data(), close.data(),
                            period, &outBeg, &outNb, outReal.data());
    if (rc != TA_SUCCESS) {
      outNb = 0;
      outBeg = 0;
    }
...
// Signature: TA_FUNC(startIdx, endIdx, inHigh[], inLow[], inClose[],
// inVolume[], &outBeg, &outNb, outReal[])
// ============================================================
template <TA_RetCode (*TA_FUNC)(int, int, const double[], const double[],
                                const double[], const double[], int *, int *,
                                double[])>
static void TalibScalarP4(DataChunk &args, ExpressionState &state,
                          Vector &result) {
  auto count = args.size();
  for (idx_t i = 0; i < count; i++) {
    auto list_h = GetListEntry(args, 0, i);
    ...
    std::vector<double> outReal(size);
    TA_RetCode rc = TA_FUNC(0, size - 1, high.data(), low.data(), close.data(),
                            volume.data(), &outBeg, &outNb, outReal.data());
    if (rc != TA_SUCCESS) {
      outNb = 0;
      outBeg = 0;
    }
...
// ============================================================
// P5 DOUBLE: (LIST<DOUBLE>, LIST<DOUBLE>, LIST<DOUBLE>, LIST<DOUBLE>) ->
// LIST<DOUBLE> Signature: TA_FUNC(startIdx, endIdx, inOpen[], inHigh[],
// inLow[], inClose[], &outBeg, &outNb, outReal[])
// ============================================================
template <TA_RetCode (*TA_FUNC)(int, int, const double[], const double[],
                                const double[], const double[], int *, int *,
                                double[])>
static void TalibScalarP5Double(DataChunk &args, ExpressionState &state,
                                Vector &result) {
  auto count = args.size();
  for (idx_t i = 0; i < count; i++) {
    ...
    int outBeg = 0, outNb = 0;
    std::vector<double> outReal(size);
    TA_RetCode rc = TA_FUNC(0, size - 1, open_.data(), high.data(), low.data(),
                            close.data(), &outBeg, &outNb, outReal.data());
    if (rc != TA_SUCCESS) {
      outNb = 0;
      outBeg = 0;
    }
...
// ============================================================
// P5 INT: (LIST<DOUBLE>, LIST<DOUBLE>, LIST<DOUBLE>, LIST<DOUBLE>) ->
// LIST<INTEGER> Signature: TA_FUNC(startIdx, endIdx, inOpen[], inHigh[],
// inLow[], inClose[], &outBeg, &outNb, outInteger[])
// ============================================================
template <TA_RetCode (*TA_FUNC)(int, int, const double[], const double[],
                                const double[], const double[], int *, int *,
                                int[])>
static void TalibScalarP5Int(DataChunk &args, ExpressionState &state,
                             Vector &result) {
  auto count = args.size();
  for (idx_t i = 0; i < count; i++) {
    auto list_o = GetListEntry(args, 0, i);
    ...
    std::vector<int> outInt(size);
    TA_RetCode rc = TA_FUNC(0, size - 1, open_.data(), high.data(), low.data(),
                            close.data(), &outBeg, &outNb, outInt.data());
    if (rc != TA_SUCCESS) {
      outNb = 0;
      outBeg = 0;
    }
...
// Signature: TA_FUNC(startIdx, endIdx, inHigh[], inLow[], &outBeg, &outNb,
// outReal[])
// ============================================================
template <TA_RetCode (*TA_FUNC)(int, int, const double[], const double[], int *,
                                int *, double[])>
static void TalibScalarP6(DataChunk &args, ExpressionState &state,
                          Vector &result) {
  auto count = args.size();
  for (idx_t i = 0; i < count; i++) {
    auto list_h = GetListEntry(args, 0, i);
    ...
    int outBeg = 0, outNb = 0;
    std::vector<double> outReal(size);
    TA_RetCode rc = TA_FUNC(0, size - 1, high.data(), low.data(), &outBeg,
                            &outNb, outReal.data());
    if (rc != TA_SUCCESS) {
      outNb = 0;
      outBeg = 0;
...
// Signature: TA_FUNC(startIdx, endIdx, inHigh[], inLow[], inClose[], &outBeg,
// &outNb, outReal[])
// ============================================================
template <TA_RetCode (*TA_FUNC)(int, int, const double[], const double[],
                                const double[], int *, int *, double[])>
static void TalibScalarP7(DataChunk &args, ExpressionState &state,
                          Vector &result) {
  auto count = args.size();
  for (idx_t i = 0; i < count; i++) {
    auto list_h = GetListEntry(args, 0, i);
...
// Signature: TA_FUNC(startIdx, endIdx, inHigh[], inLow[], optInTimePeriod,
// &outBeg, &outNb, outReal[])
// ============================================================
template <TA_RetCode (*TA_FUNC)(int, int, const double[], const double[], int,
                                int *, int *, double[])>
static void TalibScalarP8(DataChunk &args, ExpressionState &state,
                          Vector &result) {
  auto count = args.size();
  for (idx_t i = 0; i < count; i++) {
    auto list_h = GetListEntry(args, 0, i);
    ...
    int outBeg = 0, outNb = 0;
    std::vector<double> outReal(size);
    TA_RetCode rc = TA_FUNC(0, size - 1, high.data(), low.data(), period,
                            &outBeg, &outNb, outReal.data());
    if (rc != TA_SUCCESS) {
      outNb = 0;
      outBeg = 0;
...
#define TALIB_SCALAR_P1_DOUBLE(sql_name, ta_func)                              \
  loader.RegisterFunction(                                                     \
      ScalarFunction("t_" #sql_name, {LIST_DOUBLE, LogicalType::INTEGER},      \
                     LIST_DOUBLE, TalibScalarP1Double<ta_func>));

// --- P1 INT: (LIST<DOUBLE>, INTEGER) -> LIST<INTEGER> ---
#define TALIB_SCALAR_P1_INT(sql_name, ta_func)                                 \
  loader.RegisterFunction(                                                     \
      ScalarFunction("t_" #sql_name, {LIST_DOUBLE, LogicalType::INTEGER},      \
                     LIST_INT, TalibScalarP1Int<ta_func>));

// --- P2 DOUBLE: (LIST<DOUBLE>) -> LIST<DOUBLE> ---
#define TALIB_SCALAR_P2_DOUBLE(sql_name, ta_func)                              \
  loader.RegisterFunction(ScalarFunction("t_" #sql_name, {LIST_DOUBLE},        \
                                         LIST_DOUBLE,                          \
...
#define TALIB_SCALAR_P2_INT(sql_name, ta_func)                                 \
  loader.RegisterFunction(ScalarFunction(                                      \
      "t_" #sql_name, {LIST_DOUBLE}, LIST_INT, TalibScalarP2Int<ta_func>));
...
#define TALIB_SCALAR_P7_DOUBLE(sql_name, ta_func)                              \
  loader.RegisterFunction(                                                     \
      ScalarFunction("t_" #sql_name, {LIST_DOUBLE, LIST_DOUBLE, LIST_DOUBLE},  \
                     LIST_DOUBLE, TalibScalarP7<ta_func>));

// --- P8 DOUBLE: (LIST<DOUBLE> x2, INTEGER) -> LIST<DOUBLE> ---
#define TALIB_SCALAR_P8_DOUBLE(sql_name, ta_func)                              \
  loader.RegisterFunction(ScalarFunction(                                      \
      "t_" #sql_name, {LIST_DOUBLE, LIST_DOUBLE, LogicalType::INTEGER},        \
      LIST_DOUBLE, TalibScalarP8<ta_func>));
