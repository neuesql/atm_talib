...
struct TalibAggP1Double {
  ...
  static void Finalize(Vector &states_vec, AggregateInputData &, Vector &result,
    ...
    for (idx_t i = 0; i < count; i++) {
      ...
      TA_RetCode rc = TA_FUNC(0, size - 1, state->values->data(), state->period,
...
struct TalibAggP2Double {
  ...
  static void Finalize(Vector &states_vec, AggregateInputData &, Vector &result,
    ...
    for (idx_t i = 0; i < count; i++) {
      ...
      TA_RetCode rc = TA_FUNC(0, size - 1, state->values->data(), &outBeg,
                              &outNb, outReal.data());
...
struct TalibAggP3 {
  ...
  static void Finalize(Vector &states_vec, AggregateInputData &, Vector &result,
    ...
    for (idx_t i = 0; i < count; i++) {
      ...
      TA_RetCode rc = TA_FUNC(0, size - 1, state->high->data(),
...
struct TalibAggP4 {
  ...
  static void Combine(Vector &source, Vector &target, AggregateInputData &,
    ...
    for (idx_t i = 0; i < count; i++) {
      if (src[i]->high && !src[i]->high->empty()) {
        ...
        tgt[i]->close->insert(tgt[i]->close->end(), src[i]->close->begin(),
                              src[i]->close->end());
        tgt[i]->volume->insert(tgt[i]->volume->end(), src[i]->volume->begin(),
                               src[i]->volume->end());
      }
    }
  }
  ...
  static void Finalize(Vector &states_vec, AggregateInputData &, Vector &result,
    ...
    for (idx_t i = 0; i < count; i++) {
      ...
      TA_RetCode rc =
          TA_FUNC(0, size - 1, state->high->data(), state->low->data(),
                  state->close->data(), state->volume->data(), &outBeg, &outNb,
                  outReal.data());
...
struct TalibAggP5Double {
  ...
  static void Combine(Vector &source, Vector &target, AggregateInputData &,
    ...
    for (idx_t i = 0; i < count; i++) {
      if (src[i]->open_ && !src[i]->open_->empty()) {
        ...
        tgt[i]->open_->insert(tgt[i]->open_->end(), src[i]->open_->begin(),
                              src[i]->open_->end());
        tgt[i]->high->insert(tgt[i]->high->end(), src[i]->high->begin(),
                             src[i]->high->end());
        tgt[i]->low->insert(tgt[i]->low->end(), src[i]->low->begin(),
  ...
  static void Finalize(Vector &states_vec, AggregateInputData &, Vector &result,
    ...
    for (idx_t i = 0; i < count; i++) {
      ...
      TA_RetCode rc =
          TA_FUNC(0, size - 1, state->open_->data(), state->high->data(),
                  state->low->data(), state->close->data(), &outBeg, &outNb,
                  outReal.data());
...
struct TalibAggP6 {
  ...
  static void Finalize(Vector &states_vec, AggregateInputData &, Vector &result,
    ...
    for (idx_t i = 0; i < count; i++) {
      ...
      TA_RetCode rc =
          TA_FUNC(0, size - 1, state->high->data(), state->low->data(), &outBeg,
                  &outNb, outReal.data());
...
#define TALIB_AGG_P1_DOUBLE(sql_name, ta_func)                                 \
  {                                                                            \
    ...
    AggregateFunction func("ta_" #sql_name,                                    \
...
#define TALIB_AGG_P1_INT(sql_name, ta_func)                                    \
  {                                                                            \
    ...
    AggregateFunction func(                                                    \
        ...
        FunctionNullHandling::DEFAULT_NULL_HANDLING, OP::SimpleUpdate,         \
        nullptr, OP::Destroy);                                                 \
    loader.RegisterFunction(func);                                             \
  }
...
#define TALIB_AGG_P2_DOUBLE(sql_name, ta_func)                                 \
  {                                                                            \
    ...
    AggregateFunction func("ta_" #sql_name, {LogicalType::DOUBLE},             \
                           LogicalType::DOUBLE, OP::StateSize, OP::Initialize, \
                           OP::Update, OP::Combine, OP::Finalize,              \
                           FunctionNullHandling::DEFAULT_NULL_HANDLING,        \
                           OP::SimpleUpdate, nullptr, OP::Destroy);            \
...
#define TALIB_AGG_P5_INT(sql_name, ta_func)                                    \
  {                                                                            \
    ...
    AggregateFunction func("ta_" #sql_name,                                    \
                           {LogicalType::DOUBLE, LogicalType::DOUBLE,          \
                            LogicalType::DOUBLE, LogicalType::DOUBLE},         \
                           LogicalType::INTEGER, OP::StateSize,                \
