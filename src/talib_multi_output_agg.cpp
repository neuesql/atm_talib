...
struct TalibAggMinMax {
  ...
  static void SimpleUpdate(Vector inputs[], AggregateInputData &,
                           idx_t input_count, data_ptr_t state_p, idx_t count) {
  ...
  static void Combine(Vector &source, Vector &target, AggregateInputData &,
                      idx_t count) {
    ...
    for (idx_t i = 0; i < count; i++) {
      if (src[i]->values && !src[i]->values->empty()) {
        ...
        tgt[i]->values->insert(tgt[i]->values->end(), src[i]->values->begin(),
                               src[i]->values->end());
  ...
  static void Finalize(Vector &states_vec, AggregateFinalizeInputData &,
    ...
    for (idx_t i = 0; i < count; i++) {
      ...
      TA_RetCode rc =
          TA_MINMAX(0, size - 1, state->values->data(), state->period, &outBeg,
...
struct TalibAggAroon {
  ...
  static void Combine(Vector &source, Vector &target, AggregateInputData &,
    ...
    for (idx_t i = 0; i < count; i++) {
      if (src[i]->high && !src[i]->high->empty()) {
        ...
        tgt[i]->high->insert(tgt[i]->high->end(), src[i]->high->begin(),
                             src[i]->high->end());
        tgt[i]->low->insert(tgt[i]->low->end(), src[i]->low->begin(),
                            src[i]->low->end());
        tgt[i]->period = src[i]->period;
      }
    }
  ...
  static void Finalize(Vector &states_vec, AggregateFinalizeInputData &,
    ...
    for (idx_t i = 0; i < count; i++) {
      ...
      TA_RetCode rc = TA_AROON(0, size - 1, state->high->data(),
                               state->low->data(), state->period, &outBeg,
                               &outNb, outDown.data(), outUp.data());
...
struct TalibAggHtPhasor {
  ...
  static void Finalize(Vector &states_vec, AggregateFinalizeInputData &,
    ...
    for (idx_t i = 0; i < count; i++) {
      ...
      TA_RetCode rc =
          TA_HT_PHASOR(0, size - 1, state->values->data(), &outBeg, &outNb,
...
struct TalibAggHtSine {
  ...
  static void Finalize(Vector &states_vec, AggregateFinalizeInputData &,
    ...
    for (idx_t i = 0; i < count; i++) {
      ...
      TA_RetCode rc = TA_HT_SINE(0, size - 1, state->values->data(), &outBeg,
                                 &outNb, outSine.data(), outLeadSine.data());
  ...
  static void Finalize(Vector &states_vec, AggregateFinalizeInputData &,
    ...
    for (idx_t i = 0; i < count; i++) {
      ...
      TA_RetCode rc = TA_MAMA(0, size - 1, state->values->data(),
                              state->fast_limit, state->slow_limit, &outBeg,
                              &outNb, outMAMA.data(), outFAMA.data());
...
struct TalibAggMacd {
  ...
  static void Finalize(Vector &states_vec, AggregateFinalizeInputData &,
    ...
    for (idx_t i = 0; i < count; i++) {
      ...
      TA_RetCode rc =
          TA_MACD(0, size - 1, state->values->data(), state->fast_period,
                  state->slow_period, state->signal_period, &outBeg, &outNb,
  ...
  static void Finalize(Vector &states_vec, AggregateFinalizeInputData &,
    ...
    for (idx_t i = 0; i < count; i++) {
      ...
      TA_RetCode rc = TA_BBANDS(
          0, size - 1, state->values->data(), state->time_period,
          state->nb_dev_up, state->nb_dev_dn, (TA_MAType)state->ma_type,
...
struct TalibAggStoch {
  ...
  static void Combine(Vector &source, Vector &target, AggregateInputData &,
    ...
    for (idx_t i = 0; i < count; i++) {
      if (src[i]->high && !src[i]->high->empty()) {
        ...
        tgt[i]->low->insert(tgt[i]->low->end(), src[i]->low->begin(),
                            src[i]->low->end());
        tgt[i]->close->insert(tgt[i]->close->end(), src[i]->close->begin(),
                              src[i]->close->end());
  ...
  static void Finalize(Vector &states_vec, AggregateFinalizeInputData &,
    ...
    for (idx_t i = 0; i < count; i++) {
      ...
      TA_RetCode rc =
          TA_STOCH(0, size - 1, state->high->data(), state->low->data(),
                   state->close->data(), state->fastk_period,
                   state->slowk_period, (TA_MAType)state->slowk_matype,
                   state->slowd_period, (TA_MAType)state->slowd_matype, &outBeg,
                   &outNb, outSlowK.data(), outSlowD.data());
...
void RegisterTalibMultiOutputAggFunctions(ExtensionLoader &loader) {
  ...
  // ta_minmax: (DOUBLE, INT) -> STRUCT(min, max)
  {
    ...
    AggregateFunction func(
        "ta_minmax", {LogicalType::DOUBLE, LogicalType::INTEGER},
        MakeStruct(
  ...
  // ta_ht_phasor: (DOUBLE) -> STRUCT(inphase, quadrature)
  {
    ...
    AggregateFunction func("ta_ht_phasor", {LogicalType::DOUBLE},
                           MakeStruct({{"inphase", LogicalType::DOUBLE},
  ...
  // ta_ht_sine: (DOUBLE) -> STRUCT(sine, leadsine)
  {
    ...
    AggregateFunction func("ta_ht_sine", {LogicalType::DOUBLE},
                           MakeStruct({{"sine", LogicalType::DOUBLE},
  ...
  // ta_macd: (DOUBLE, INT, INT, INT) -> STRUCT(macd, signal, hist)
  {
    ...
    AggregateFunction func("ta_macd",
                           {LogicalType::DOUBLE, LogicalType::INTEGER,
                            LogicalType::INTEGER, LogicalType::INTEGER},
                           MakeStruct({{"macd", LogicalType::DOUBLE},
  ...
  // ta_bbands: (DOUBLE, INT, DOUBLE, DOUBLE, INT) -> STRUCT(upper, middle,
  // lower)
  {
    ...
    AggregateFunction func(
        ...
        {LogicalType::DOUBLE, LogicalType::INTEGER, LogicalType::DOUBLE,
         LogicalType::DOUBLE, LogicalType::INTEGER},
        MakeStruct({{"upper", LogicalType::DOUBLE},
